...
void test_encoding_training_from_loaded_file(const int batch_size,
  ...
  CHECK(areVectorsWithinTolerance(
      tnn::Module::convertTensorToVector<T>(torch_params), params, epsilon));
  ...
  std::vector<T> output_vec =
      tnn::Module::convertTensorToVector<T>(output_encoding);
  ...
  CHECK(isVectorWithinTolerance(encoding.get_encoding_grads(), 0.0f,
                                epsilon));  // default init to 0.0f

  auto [dL_dinput_empty, torch_grad] = encoding.backward_pass(
      tnn::Module::convertDeviceMatrixToTorchTensor<T>(dL_doutput),
      input_encoding);
  ...
  CHECK(areVectorsWithinTolerance(
      tnn::Module::convertTensorToVector<T>(torch_grad), enc_grads, epsilon));
...
void test_encoding_with_no_params_forward_from_loaded_file(
  ...
  auto output_tensor = encoding_module.forward_pass(
      tnn::Module::convertDeviceMatrixToTorchTensor(input));
...
TEST_CASE("EncodingModule::Identity") {
  ...
  SUBCASE("forward_pass with Identity encoding") {
    ...
    // Call forward_pass (use_inference flag has been set to false as an
    // example)
  ...
  SUBCASE("Not padded") {
    ...
    auto input_tensor =
        torch::full({batch_size, input_width}, 1.23).to(torch::kXPU);
    auto output_tensor = encoding_module.forward_pass(input_tensor);
...
TEST_CASE("EncodingModule::Spherical Harmonics") {
  ...
  SUBCASE("Not padded") {
    ...
    const std::vector<float> reference_out = {0.2821,  -0.4886, 0.4886,
                                              -0.4886, 1.0925,  -1.0925,
                                              0.6308,  -1.0925, 0.0000};
...
TEST_CASE("EncodingModule::Grid Encoding") {
  SUBCASE("Test grid encoding using create_grid_encoding instead of factory") {
    ...
    const std::vector<float> reference_out = {
        1.1325,  -4.1187, 1.0490,  -4.0174, 0.5364, 0.7808, -3.2306, 3.6359,
    ...
    auto output_vec =
        tnn::Module::convertTensorToVector<float>(output_tensor / 1e-05);
