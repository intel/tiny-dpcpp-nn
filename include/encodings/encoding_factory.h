...
class EncodingFactory {
 public:
  ...
  virtual std::shared_ptr<Encoding<T>> create(
      const json &config, std::optional<uint32_t> padded_output_width,
      sycl::queue &Q) const = 0;
};
...
class IdentityEncodingFactory : public EncodingFactory<T> {
 public:
  std::shared_ptr<Encoding<T>> create(
      const json &config, std::optional<uint32_t> padded_output_width,
      sycl::queue &Q) const override {
    ...
    if (!config.contains(EncodingParams::N_DIMS_TO_ENCODE))
      throw std::invalid_argument("Config misses " +
    ...
    return std::make_shared<IdentityEncoding<T>>(
        n_dims_to_encode,
        padded_output_width.has_value() ? padded_output_width.value()
                                        : n_dims_to_encode,
        scale, offset, Q);
  }
};
...
class SphericalHarmonicsEncodingFactory : public EncodingFactory<T> {
 public:
  std::shared_ptr<Encoding<T>> create(
    ...
    const uint32_t n_dims_to_encode = config[EncodingParams::N_DIMS_TO_ENCODE];
    return std::make_shared<SphericalHarmonicsEncoding<T>>(
        degree, n_dims_to_encode,
        padded_output_width.has_value() ? padded_output_width.value()
                                        : (degree * degree),
        Q);
  }
 ...
 public:
  std::shared_ptr<Encoding<T>> create(
    ...
    static_assert(std::is_same<T, float>::value,
                  "GridEncodingFactory only supports float");

    return tinydpcppnn::encodings::grid::create_grid_encoding<T>(
        config, padded_output_width, Q);
  }
};

// EncodingFactory for GridEncodingTemplated
template <typename T>
class FrequencyEncodingFactory : public EncodingFactory<T> {
 public:
  std::shared_ptr<Encoding<T>> create(
      const json &config, std::optional<uint32_t> padded_output_width,
      sycl::queue &Q) const override {
    ...
    if (std::numeric_limits<uint32_t>::max() / n_frequencies <
        2 * n_dims_to_encode)
      throw std::invalid_argument(
          "n_frequencies * 2 * n_dims_to_encode exceeds uint32_t max");

    return std::make_shared<FrequencyEncoding<T>>(
        n_frequencies, n_dims_to_encode,
        padded_output_width.has_value() ? padded_output_width.value()
                                        : 2 * n_frequencies * n_dims_to_encode,
        Q);
  }
};

template <typename T>
std::shared_ptr<Encoding<T>> create_encoding(
  ...
  auto check_config = [&](const json &config) {
    if (!config.contains(EncodingParams::ENCODING))
      throw std::invalid_argument("Config needs to contain encoding, key " +
      ...
      if (name != EncodingNames::IDENTITY &&
          ...
          name != EncodingNames::GRID && name != EncodingNames::FREQUENCY)
        throw std::invalid_argument("Unknown encoding type: " + name);
    }

    const std::unordered_set<std::string> valid_keys(
        {EncodingParams::ENCODING, EncodingParams::N_DIMS_TO_ENCODE,
         EncodingParams::GRID_TYPE, EncodingParams::N_LEVELS,
         EncodingParams::N_FEATURES, EncodingParams::N_FEATURES_PER_LEVEL,
         EncodingParams::LOG2_HASHMAP_SIZE, EncodingParams::BASE_RESOLUTION,
         EncodingParams::PER_LEVEL_SCALE, EncodingParams::DEGREE,
         EncodingParams::SCALE, EncodingParams::OFFSET, EncodingParams::HASH,
    ...
    const bool valid =
        std::all_of(config.items().begin(), config.items().end(),
                    [&valid_keys](const auto &config_key) {
                      return valid_keys.contains(config_key.key());
                    });

    if (!valid) {
      std::cout << "WARNING: found unknown option in encoding config. Please "
                   "ensure that your options are one of the following:\n.";
      std::for_each(valid_keys.begin(), valid_keys.end(),
